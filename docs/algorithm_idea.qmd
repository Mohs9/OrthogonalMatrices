...
The main idea of the algorithm is to use small perturbations of an orthogonal matrix $Q$ that satisfies @eq-optimizationProblem. In this context, $f(Q) = \kappa_p(PQ)$, with the constraint $QQ'= I$. However, since this equation is subject to the constraint $QQ'=I$, a small modification must be made to the gradient $\nabla f(Q)$ that would commonly be used to take a small step toward a local minimum.
...
Hence, in this section we present a brief description of the algorithm to achieve the @eq-optimizationProblem.
...
since $(LQ)^{-1}=Q'L^{-1}$ and the logarithm preserves the minimizer (idea of Riemman Manifolds is the same).
